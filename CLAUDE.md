...
- When ECC is available in the active environment, use it as the default development harness for the full lifecycle: `/ecc:plan` → `/ecc:tdd-workflow` when behavior changes → `/ecc:code-review` → `/ecc:verification-loop`, with `/ecc:build-fix` only after an actual build/type failure and `/ecc:update-docs` when source-of-truth docs need synchronization. Follow [AGENTS.md](AGENTS.md) for project boundaries, minimal checks, and documented exceptions. ECC may assist with upstream conflict analysis, but the developer must manually confirm the final merge and any `git add`/`git commit`.
