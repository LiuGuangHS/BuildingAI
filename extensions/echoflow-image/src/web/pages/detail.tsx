...
export default function DetailPage() {
    ...
    const { id = "" } = useParams();
    const { data, isLoading, isError, refetch } = useWebGenerationDetailQuery(id, {
        staleTime: 0,
        refetchInterval: (query) => {
            const status = (query.state.data as ImageGeneration | undefined)?.status;
            return status === ImageGenerationStatus.PENDING || status === ImageGenerationStatus.PROCESSING
                ? 2000
                : false;
        },
    });
    ...
    if (isError) {
        return (
            <div className="p-4 md:p-6">
                <ErrorState title="加载详情失败" message="无法获取生成记录详情，请检查后重试" onRetry={() => refetch()} />
            </div>
        );
    }
        ...
        <div className="space-y-6 p-4 md:p-6">
            <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
                ...
                <div className="flex gap-2">
                    ...
                    <Button variant="destructive" loading={deleteMutation.isPending} onClick={() => setShowDeleteConfirm(true)}>
                    ...
                    <Card>
                        ...
                        <CardContent className="space-y-4 text-sm">
                            <div>
                                ...
                                <p className="whitespace-pre-wrap rounded-lg bg-muted/50 p-3 leading-relaxed">{data.prompt}</p>
                            </div>
                            {data.negativePrompt && (
                                <div>
                                    <div className="text-muted-foreground mb-1 text-xs font-medium">反向提示词</div>
                                    <p className="whitespace-pre-wrap rounded-lg bg-muted/50 p-3 leading-relaxed">{data.negativePrompt}</p>
                                </div>
                            )}
                            {isSafeImageSrc(data.referenceImageUrl) && (
                                <div>
                                    <div className="text-muted-foreground mb-1 text-xs font-medium">参考图</div>
                                    <img src={data.referenceImageUrl} alt="参考图" className="max-h-72 rounded-xl border object-contain" />
                                </div>
                            )}
                            {isSafeImageSrc(data.maskImage?.url) && (
                                <div>
                                    <div className="text-muted-foreground mb-1 text-xs font-medium">遮罩图</div>
                                    <img src={data.maskImage.url} alt="遮罩图" className="max-h-72 rounded-xl border object-contain" />
                                </div>
                            )}
