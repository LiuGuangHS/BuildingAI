...
export class Upgrade {
    ...
    async execute(): Promise<void> {
        ...
        await this.ensureProviderRuntimeColumns();
        await this.ensureExtensionRecord();
        this.logger.log("Echoflow Video upgrade to version 0.0.7 completed");
    }
    ...
    private async ensureProviderRuntimeColumns(): Promise<void> {
        ...
        await this.dataSource.query(`
            ...
            ADD COLUMN IF NOT EXISTS "base_url" varchar(500) NOT NULL DEFAULT 'https://api.echoflow.cn'
        `);
        await this.dataSource.query(`
            ALTER TABLE "echoflow_video"."video_provider_config"
            ADD COLUMN IF NOT EXISTS "request_timeout_ms" integer NOT NULL DEFAULT 120000
        `);
        await this.dataSource.query(`
            ALTER TABLE "echoflow_video"."video_provider_config"
            ADD COLUMN IF NOT EXISTS "test_timeout_ms" integer NOT NULL DEFAULT 15000
        `);
        await this.dataSource.query(`
            ALTER TABLE "echoflow_video"."video_provider_config"
